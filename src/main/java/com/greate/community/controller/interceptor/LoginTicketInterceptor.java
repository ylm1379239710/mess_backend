    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (ticket != null) {
            ...
            if (loginTicket != null && loginTicket.getStatus() == 0 && loginTicket.getExpired().after(new Date())) {
                ...
                ActionResult actionResult = ResultBuilder.buildActionFail(HttpStatus.UNAUTHORIZED.value(),null, "登录状态已失效或令牌不存在");
                response.getWriter().print(JSON.toJSONString(actionResult));
                return false;
            }
        }else {
            response.setContentType("application/json;charset=utf-8");
            ActionResult actionResult = ResultBuilder.buildActionFail(HttpStatus.UNAUTHORIZED.value(),null, "还未登录！");
            response.getWriter().print(JSON.toJSONString(actionResult));
            return false;
        }
