...
public class UserController implements CommunityConstant {
    ...
    public ActionResult getMyComments(@CookieValue("ticket") String ticket) {
        LoginTicket loginTicket = findLoginTicket(ticket);
        User user = userService.findUserById(loginTicket.getUserId());
        if (user == null) {
            return ResultBuilder.buildActionFail(null,"该用户不存在！");
        }else {
            return ResultBuilder.buildActionSuccess(user);
        }
    }
