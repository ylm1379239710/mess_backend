...
public class UserService implements UserDetailsService, CommunityConstant {
    ...
    public Map<String, Object> register(User user) {
        ...
        context.setVariable("qq", "https://jq.qq.com/?_wv=1027&k=Uaqhyb1e");
        context.setVariable("username", username);
        context.setVariable("password", password);
        String content = templateEngine.process("mail/activation", context);
        mailClient.sendMail(user.getEmail(), "激活 Mess 账号", content);
    ...
    public Map<String, Object> doSendEmailCode4ResetPwd(String account) {
        ...
        // http://localhost:8080/echo/activation/用户id/激活码
        String url = domain + contextPath + "/activation/" + user.getId() + "/" + user.getActivationCode();
        context.setVariable("password", url);
        String content = templateEngine.process("/mail/forgetPassword", context);
        mailClient.sendMail(email, "重置 Mess 账号密码", content);
